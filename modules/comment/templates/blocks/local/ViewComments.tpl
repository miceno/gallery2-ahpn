...
<div class="{$class}">
  <div class="BlockTitle">
     <h3 style="display:inline">{g->text text="Recent comments"}</h3> {if sizeof($block.comment.LoadComments.comments)< $block.comment.LoadComments.totalComments }<a href="{g->url arg1="view=comment.ShowComments" arg2="itemId=`$block.comment.LoadComments.item.id`"}">{g->text text="(view all %d comments)" arg1=$block.comment.LoadComments.totalComments}</a>{/if}
  </div>
