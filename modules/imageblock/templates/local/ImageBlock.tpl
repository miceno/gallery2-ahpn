...
<div class="one-image">
  ...
  {else}
    {$link}
      {g->image item=$imageItem image=$block.thumb class="giThumbnail reflect" maxSize=$maxSize forceFullUrl=$ImageBlockData.forceFullUrl}
    {if !empty($linkHref)} </a> {/if}
  {/if}

  {if isset($ImageBlockData.show.title) && isset($block.item.title)}
    <h3 class="giTitle">{$link}{$block.item.title|markup}{if !empty($linkHref)}</a>{/if}</h3>
  {/if}
