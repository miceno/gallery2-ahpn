...
<div class="gbBlock">
  <p class="giDescription">
    ...
    <a onclick="document.getElementById('ItemAddBulkItemUpdate_instructions').style.display='block'">{g->text text="[help]"}</a>
  </p>
  <p style="display: none" id="ItemAddBulkItemUpdate_instructions" class="giDescription">
    {capture assign=sampleDataFile}<a href="{g->url href="modules/bulkitemupdate/data/sample.txt"}">{/capture}
    {capture assign=sampleExcelSpreadsheet}<a href="{g->url href="modules/bulkitemupdate/data/sample.xls"}">{/capture}
    {g->text text="The photos you want to update must already be in the gallery.  Create a data file containing the title and the custom fields for each photo, and then enter the path to the text file in the box below.  For convenience, you can author the data file in Excel and then save it in the %sText (tab delimited)%s format.  Here is a %ssample data file%s and a %ssample excel spreadsheet%s." arg1="<b>" arg2="</b>" arg3=$sampleDataFile arg4="</a>" arg5=$sampleExcelSpreadsheet arg6="</a>"}
  </p>
