...
{/if}
  <div id="legend" class="gcBackground1" style="{if $map.LegendPos eq 'right'}right:5px;{if $map.ThumbBarPos eq 'top'}position:relative;top:-{$map.ThumbHeight}px;{/if}{/if}{if $map.LegendPos eq 'right' or $map.LegendPos eq 'left'}width:150px;{else}width:mywidth;{/if}border: 2px solid black;padding:.5em;">
    ...
    {if $map.LegendPos eq 'right' or $map.LegendPos eq 'left'}
      ...
      <tr><td colspan=3>
    {/if}
    <table {if $map.LegendPos eq 'right' or $map.LegendPos eq 'left'}id="albumlegend" {if $map.ExpandAlbumLegend}style="display:block;"{else}style="display:none;"{/if}{/if}>
    {if $map.LegendPos eq 'top' or $map.LegendPos eq 'bottom'}<tr><td>{g->text text="Album Legend"}</td>{/if}
    ...
    {foreach from=$imagelist key=name item=images}
       ...
       {if $map.AlbumLegends.$num neq ""}
         {if $map.LegendPos eq 'right' or $map.LegendPos eq 'left'}<tr>{/if}
    ...
    {if $map.LegendPos eq 'right' or $map.LegendPos eq 'left'}</td></tr></table>{/if}
    {/if}
    ...
    <tr><td colspan=3>
    {/if}
    <table {if $map.LegendPos eq 'right' or $map.LegendPos eq 'left'}id="photolegend" {if $map.ExpandAlbumLegend}style="display:block;"{else}style="display:none;"{/if}{/if}>
    {if $map.LegendPos eq 'top' or $map.LegendPos eq 'bottom'}<tr><td>{g->text text="Photo Legend"}</td>{/if}
    {foreach from=$map.allmarkers key=num item=imagelist}
    {foreach from=$imagelist key=name item=images}
    ...
    <img alt="none" src="{g->url href="modules/mapv3/images/multi/{$map.regroupIcon}.png"}"/>
    {else}
    <img alt="none" src="{g->url href="modules/mapv3/images/blank.gif"}" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='{g->url href="modules/mapv3/images/multi/"}{$map.regroupIcon}.png');"/>
    {/if}
